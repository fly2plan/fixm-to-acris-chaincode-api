...
import { buildCCPOrg } from './AppUtil'
...
export class UtilsConfig {
    ...
    public async registerAndEnrollUser(caClient: FabricCAServices, wallet: Wallet, orgMspId: string, userId: string, affiliation: string, orgType: string, iataCode: string, password: string): Promise<string> {
        try {
            ...
            const secret = await caClient.register({
                affiliation,
                enrollmentID: userId,
                role: 'client', attrs: [{ name: 'orgtype', value: orgType, ecert: true }, { name: 'iatacode', value: iataCode, ecert: true }, { name: 'password', value: password, ecert: true }]
            }, adminUser);
    ...
    *@return : boolean .if user enrolled correctly , true.
    */
    public async isUserExits(caClient: FabricCAServices, wallet: Wallet, userId: string, userPassword:string): Promise<boolean> {
        try {
            ...
            if(ConfigContants.DISCOVERY_AS_LOCALHOST.toString() === 'false') return false;
            ...
            const response = await identityService.getOne(userId.trim(), adminUser);
            gateway.disconnect();
            let password = "";
            if (response.result.attrs) {
                const arryAttr = response.result.attrs;
                const keys = Object.keys(response.result.attrs);
            ...
            if(password.trim() === userPassword.trim()) return true; else return false;
