...
export class AppLogger implements LoggerService {
    ...
    initializeLogger(label?: string) {
        this.info = winston.createLogger({
            ...
            transports: [
                // new winston.transports.File({ dirname: config.getLogPath(), filename: 'error.log', level: 'error' }),
                new winston.transports.File({ dirname: config.getLogPath(), filename: 'info.log', level: 'info' }),
                // new winston.transports.File({ dirname: config.getLogPath(), filename: 'api.log', level: 'http' }),
            ],
        });
